...
REPO_PATH := github.com/Microsoft/oe-engine
...
build-binary: generate
	go build $(GOFLAGS) -v -ldflags "${LDFLAGS}" -o ${BINARY_DEST_DIR}/oe-engine .
...
build-cross:
	CGO_ENABLED=0 gox -output="_dist/oe-engine-${GITTAG}-{{.OS}}-{{.Arch}}/{{.Dir}}" -osarch='$(TARGETS)' $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)'
...
ifneq ($(GIT_BASEDIR),)
	LDFLAGS += -X github.com/Microsoft/oe-engine/pkg/test.JUnitOutDir=${GIT_BASEDIR}/test/junit
endif
