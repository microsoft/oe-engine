...
{{if .IsWindows}}
    ...
    {{if IsPublic .Ports}}
        "{{.Name}}VMNamePrefix": "[concat('oe-pb', variables('winResourceNamePrefix'), add(900,variables('{{.Name}}Index')))]",
        "{{.Name}}windowsAgentCustomAttributes": "",
        "{{.Name}}windowsAgentCustomScriptArguments": "",
    {{else}}
        "{{.Name}}VMNamePrefix": "[concat('oe-pr', variables('winResourceNamePrefix'), add(900,variables('{{.Name}}Index')))]",
        "{{.Name}}windowsAgentCustomAttributes": "",
        "{{.Name}}windowsAgentCustomScriptArguments": "",
    {{end}}
    "{{.Name}}windowsAgentCustomScript": "[concat('powershell.exe -ExecutionPolicy Unrestricted -command \"', variables('{{.Name}}windowsAgentCustomScriptArguments'), variables('windowsCustomScriptSuffix'), '\" > %SYSTEMDRIVE%\\AzureData\\WindowsProvision.log 2>&1; exit $LASTEXITCODE')]",
{{else}}
